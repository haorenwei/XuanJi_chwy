...
## 简介
本文件面向AI开发者与系统架构师，系统性解析XuanJi多智能体系统的深度技术实现。重点涵盖Agent核心架构、四象智能体（晴、焕、遥、玄）的协作机制与角色分工、璇玑四象AI技能实现细节（意图识别、情感分析、风格设计、对话生成）、智能体间通信协议与上下文传递、结果整合流程、AI模型集成与提示词工程、推理链设计、性能优化与错误处理策略，并提供调试技巧与最佳实践。
...
## 依赖分析
- 组件耦合：Agent对服务层与LLM客户端强依赖；四象角色通过统一提示词加载器与工厂方法解耦。
...
## 结论
XuanJi通过“多智能体协作+工具链闭环”的架构，实现了从意图识别、情绪感知、风格设计到对话生成的完整链路。四象角色职责清晰、边界明确，配合统一的提示词工程与模型集成方案，既保证了系统稳定性，也提供了良好的可扩展性与可演进性。通过并行化、流式输出、缓存与降级策略，系统在性能与用户体验上取得良好平衡。

## 附录
- 提示词工程：四象SKILL规范定义了严格的输出格式与行为约束，建议在新增或调整角色能力时，优先完善SKILL与analysis-framework，再在Agent中注入系统提示。
