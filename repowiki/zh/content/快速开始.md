...
- Python 3.11+、Node.js 18+、MySQL 8.0+ 的安装与配置
- Conda 环境创建与依赖安装
- 数据库初始化与 Alembic 迁移
- 前端与后端的启动步骤、热重载与代理配置
- 常见环境问题与解决方案
...
- 配置中心：数据库连接、密钥与跨域配置（AI角色的LLM配置唯一入口为前端「记忆体」页面，不在配置文件中设置）
...
- **注意：** AI角色（玄/晴/焕/遥/机）的LLM配置不在此处，唯一入口为前端「记忆体」页面
...
- 复合工具迁移脚本：为工具表新增列、创建组合工具与版本表（数据库表保留，工具执行功能已停用）
...
## 结论
按照本指南，您可以在约30分钟内完成环境搭建与首次运行。启动后，请前往前端「记忆体」页面配置至少玄（对话AI）的LLM参数以开始对话，再探索 AI 技能模块与系统协作流程。
...
- 安装前置软件
  ...
  - Node.js 18+（与 package.json 中版本兼容）
  - MySQL 8.0+（确保可连接并具备创建数据库权限）
...
- 配置数据库
  - 在 .env 配置文件中确认数据库连接参数（AI角色的LLM配置在启动后通过前端「记忆体」页面设置）
  - 启动后端应用以自动创建数据库与表结构
