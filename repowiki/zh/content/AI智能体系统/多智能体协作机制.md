...
- 机（系统迭代）：后台独立运行，负责规则引擎迭代、对话质量自评、画像监控、身份迭代、流程效率分析。不参与用户对话流程。
...
- 协作流程
  ...
  - 机：后台独立运行系统迭代任务（规则引擎、对话质量、画像监控、身份迭代、流程效率），不参与对话流程。
...
JUDG -- 否 --> P["并行：焕：情绪分析"] --> E3["遥：风格设计"] --> E4["玄：生成回复"] --> D2["done + 协作日志"]
P --> D3["最终汇聚情绪快照/规则贡献"]
D1 --> END["结束"]
D2 --> END
D3 --> END
